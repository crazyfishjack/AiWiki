...
category: tool
tags: [jina, reader-api, search-api, embeddings, rerank, deep-research, retrieval, readerlm-v2, anti-hallucination]
sources:
  - https://docs.jina.ai/
...
language: zh-CN+en
discovery:
  one_liner: "四类 Jina API 完整调用指南 + readerlm-v2 幻觉风险路由决策"
  ...
  key_outputs:
    ...
    - "Deep Research 三场景（精读/补充搜索/重排序）的质量优先标准配置与 Reader vs Search 选择表"
  estimated_tokens: 2200
  sections:
    - heading: "What It Is / 是什么"
      one_liner: "Jina AI 四类核心 API 简介及其在 Deep Research v2 SOP 中的专用定位"
      lines: [4, 16]
      tokens: 200
    ...
    - heading: "⚠️ readerlm-v2 幻觉风险与 URL 类型路由"
      ...
      tokens: 450
    - heading: "与竞品对比（2026）"
      one_liner: "Jina/Firecrawl/Crawl4AI/Trafilatura 幻觉风险与中文站适用性对比及当前 SOP 立场"
      lines: [227, 243]
      tokens: 200
    - heading: "Deep Research 工作流中的使用策略"
      one_liner: "精读/补充搜索/重排序三场景配置 + Reader vs Search 情景选择速查表"
      lines: [245, 289]
      tokens: 400
---
...
**在 Deep Research 工作流中的定位（v2 SOP 更新）**：Jina 是**学术 PDF 精读专用工具 + Reranker**——普通页面精读已由 Firecrawl 接管；Jina 仅保留两个专用场景：① arXiv / 学术期刊 / IEEE 等结构化学术页面精读（readerlm-v2 在此类页面有结构理解优势）；② 候选文档超量（>15篇）时的 Reranker 重排序。
...
|------|---------|---------|-------------|---------|
| **Jina Reader**（默认） | 单页精读，API 最简 | 低（不开 readerlm）| ✅ 可用 | 三工具 SOP 精读层首选 |
...
**当前 SOP 立场**：三工具 SOP 继续以 Jina Reader（默认配置）作为精读层首选，
条件：不开 `readerlm-v2` + 按 URL 类型路由。中文内容聚合站使用 Tavily `raw_content` 替代 Jina 精读。
...
### 场景一：精读目标页面
```
Tavily /search → 获取 top URL 列表
Jina r.jina.ai → 对每个重要 URL 提取完整 Markdown 内容
```
...
| 语义搜索（非关键词） | `s.jina.ai` |
| 快速预览静态页面 | `crawlSinglePage("r.jina.ai/"+URL)`（备用）|
