...
## 风险与约束（衔接点、上下文、测试、规格漂移等风险）

## 信源质量门控记录（保留信源、剔除信源、来源等级）

## 来源与可信度（关键来源、来源类型、可信度）

## 对于可信度较高的来源逐来源总结（逐篇整理高可信来源的核心内容）

## 跨源矛盾检测结论（检测核心数字、日期、功能描述等矛盾）
...
### 规则 1：结构镜像
- 证据包中的每个章节、子章节，总结中必须对应存在
- 禁止跳过任何章节，即使内容重复也要保留
- 证据包中的层级结构（##、###、####）必须在总结中保持一致
...
- [ ] 所有表格是否保留？
- [ ] 所有数据是否保留具体数值？
- [ ] 所有技术概念是否有定义+示例+实现+关系？
- [ ] 是否有"包括但不限于"等模糊表述？

如果任何一项未通过，必须补充展开。

### 规则 8：优先级
如果内容超出 max_tokens，按以下优先级压缩：
1. 保留：技术细节、数据指标、架构描述、实现方式
2. 压缩：背景介绍、历史沿革、市场分析
3. 删除：重复内容、过时的信息、明显无关的内容
